...
class MemberOnlyIndex(Resource):
    def get(self):
        ...
        if user_id:
            articles = [
                a.to_dict() for a in Article.query.filter_by(is_member_only=True).all()
            ]

            response = make_response(jsonify(articles), 200)
            return response

        elif not user_id:  # Use 'user_id' instead of 'session["user_id"]'
            return {}, 401
...
class MemberOnlyArticle(Resource):
    def get(self, id):
        ...
        if user_id:
            ...
            article = [a for a in articles if a.id == id]  # Fix the variable name

            if len(article) == 1:
                article_dict = article[0].to_dict()
                response = make_response(jsonify(article_dict), 200)
                return response
            else:
                return {}, 404  # Handle the case when no or multiple articles match
